...
def main():
    ...
    with open(os.path.join(WORK_SCRATCH, 'apply_cyphers_v3.sql'), 'w', encoding='utf-8') as f:
        ...
        f.write(f"\nUPDATE events SET registration_count = (SELECT COUNT(*) FROM registrations WHERE event_id = '{CYPHERS_EVENT_ID}') WHERE id = '{CYPHERS_EVENT_ID}';\n")

    print(f"Total: 59 | Matches: {len(matches)} | Missing: {len(missing)}")

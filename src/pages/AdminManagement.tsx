...
export default function AdminManagement() {
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Gestão de Administradores</h1>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {profiles.map((profile) => (
                <tr key={profile.id} className="hover:bg-sky-50/30 transition-colors group">
                  <td className="px-4 md:px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-sky-500 text-black flex items-center justify-center font-black text-lg shadow-sm">
                  ...
                  <td className="px-4 md:px-8 py-5">
                    <div className="flex items-center gap-3">
                       <span className={`w-2 h-2 rounded-full ${
                         profile.status === 'approved' ? 'bg-green-500 shadow-sm' :
                         profile.status === 'pending' ? 'bg-amber-500 animate-pulse shadow-sm' :
                  ...
                  <td className="px-4 md:px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-3">
                      ...
                      {profile.status === 'blocked' && (
                        <button
                          onClick={() => updateStatus(profile.id, 'approved')}
                          className="w-10 h-10 bg-sky-500/10 text-sky-600 hover:bg-sky-500 hover:text-black rounded-xl flex items-center justify-center transition-all border border-sky-500/10 shadow-sm"
